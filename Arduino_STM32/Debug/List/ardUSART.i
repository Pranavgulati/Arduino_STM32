...
extern "C++" {
  _Pragma("function_effects = no_state, no_errno, no_write(1)")    __intrinsic __nounwind const void *memchr(const void *_S, int _C, 
                                                 size_t _N);
  _Pragma("function_effects = no_state, no_errno, no_write(1)")    __intrinsic __nounwind const char *strchr(const char *_S, int _C);
  _Pragma("function_effects = no_state, no_errno, no_write(1,2)") __intrinsic __nounwind const char *strpbrk(const char *_S, 
                                                  const char *_P);
  _Pragma("function_effects = no_state, no_errno, no_write(1)")    __intrinsic __nounwind const char *strrchr(const char *_S, int _C);
  _Pragma("function_effects = no_state, no_errno, no_write(1,2)") __intrinsic __nounwind const char *strstr(const char *_S, 
                                                 const char *_P);
  _Pragma("function_effects = no_state, no_errno, no_write(1)")    __intrinsic __nounwind void *      memchr(void *_S, int _C, size_t _N);
  _Pragma("function_effects = no_state, no_errno, no_write(1)")    __intrinsic __nounwind char *      strchr(char *_S, int _C);
  _Pragma("function_effects = no_state, no_errno, no_write(1,2)") __intrinsic __nounwind char *      strpbrk(char *_S, const char *_P);
  _Pragma("function_effects = no_state, no_errno, no_write(1)")    __intrinsic __nounwind char *      strrchr(char *_S, int _C);
  _Pragma("function_effects = no_state, no_errno, no_write(1,2)") __intrinsic __nounwind char *      strstr(char *_S, const char *_P);
}
...
void serial::stopAll(){
USART_DeInit(((USART_TypeDef *) (((uint32_t)0x40000000) + 0x00013800)));
USART_DeInit(((USART_TypeDef *) (((uint32_t)0x40000000) + 0x00004400)));
USART_DeInit(((USART_TypeDef *) (((uint32_t)0x40000000) + 0x00004800)));
USART_DeInit(((USART_TypeDef *) (((uint32_t)0x40000000) + 0x00004C00)));
}
...
void serial::begin(uint8_t COMPORT,uint32_t baudRate){
  ...
  txBuf_tail=0;
  txBuf_head=0;
  USART_TypeDef* USART =((USART_TypeDef *) (((uint32_t)0x40000000) + 0x00013800));
  USART_InitTypeDef USART_properties;
  GPIO_TypeDef* gpioPort=((GPIO_TypeDef *) ((((uint32_t)0x40000000) + 0x08000000) + 0x00000000));
  uint8_t RXpin=10,TXpin=9;
  uint32_t usartClock=((uint32_t)0x00004000);
  
  switch(COMPORT){
    case COM1:
      USART =((USART_TypeDef *) (((uint32_t)0x40000000) + 0x00013800));
      gpioPort=((GPIO_TypeDef *) ((((uint32_t)0x40000000) + 0x08000000) + 0x00000000));
      RXpin=10;
      TXpin=9;
      usartClock=((uint32_t)0x00004000);
      break;
    case COM2:
      USART =((USART_TypeDef *) (((uint32_t)0x40000000) + 0x00004400));
      gpioPort=((GPIO_TypeDef *) ((((uint32_t)0x40000000) + 0x08000000) + 0x00000000));
      RXpin=3;
      TXpin=2;
      usartClock=((uint32_t)0x00020000);
      break;
    default:
      
      break;
    }  
  pinMode(gpioPort,TXpin,GPIO_Mode_AF,GPIO_PuPd_UP,GPIO_Speed_Level_2,GPIO_OType_PP);
  pinMode(gpioPort,RXpin,GPIO_Mode_AF,GPIO_PuPd_UP,GPIO_Speed_Level_2,GPIO_OType_PP);
  
  
  RCC_APB2PeriphClockCmd(usartClock, ENABLE); 
  
  pinAFconfig(gpioPort,TXpin, ((uint8_t)0x01));
  pinAFconfig(gpioPort,RXpin, ((uint8_t)0x01));
  ...
  USART_ITConfig(Serial.USART,((uint32_t)0x00050105),  ENABLE);
...
void serial::write(uint8_t *data,uint32_t size){
  ...
  for(int i=0;i<size;i++){
    ...
    uint8_t next = (Serial.txBuf_tail + 1) % 128;
    int temp=USART_GetITStatus(Serial.USART, ((uint32_t)0x00070107))==SET;
    if(Serial.txBuf_head== Serial.txBuf_tail && USART_GetITStatus(Serial.USART, ((uint32_t)0x00070107))==SET ){
      USART_SendData(Serial.USART,Serial.txBuf[Serial.txBuf_head]);     
    }
    else if (next != Serial.txBuf_head)
    {
       
      Serial.txBuf[Serial.txBuf_tail] = (*(data++)); 
    ...
    USART_ITConfig(Serial.USART,((uint32_t)0x00070107),ENABLE);
    USART_Cmd(Serial.USART,ENABLE);
...
if (Serial.recvBuf_head == Serial.recvBuf_tail){ return -1;}
  ...
  uint8_t d = Serial.recvBuf[Serial.recvBuf_head]; 
  Serial.recvBuf_head = (Serial.recvBuf_head + 1) % 128;
  return d;
}
uint8_t serial::available(){
  return (Serial.recvBuf_tail + 128 - Serial.recvBuf_head) % 128;
}
