...
void USART1_IRQHandler(void){
  ...
  if(USART_GetITStatus(Serial.USART, ((uint32_t)0x00050105)) != RESET ){
    USART_ClearFlag(Serial.USART,((uint32_t)0x00050105));
    
  uint16_t d = 0;
    
    d=(uint16_t)(Serial.USART->RDR & (uint16_t)0x01FF);;
    
    uint8_t next = (Serial.recvBuf_tail + 1) % 128;
    if (next != Serial.recvBuf_head)
    {
      
      Serial.recvBuf[Serial.recvBuf_tail] = (uint8_t)d; 
 ...
 if(USART_GetITStatus(Serial.USART, ((uint32_t)0x00070107)) != RESET){
    
    USART_ClearFlag(Serial.USART,((uint32_t)0x00000040));
    USART_ClearITPendingBit(Serial.USART,  ((uint32_t)0x00070107));
    if (Serial.txBuf_head == Serial.txBuf_tail){ USART_ITConfig(Serial.USART,((uint32_t)0x00070107),DISABLE);return ;}
    else{
      unsigned char c = Serial.txBuf[Serial.txBuf_head];
      Serial.txBuf_head = (Serial.txBuf_head + 1) % 128;  
...
*__ADC_data= ADC_GetConversionValue(((ADC_TypeDef *) (((uint32_t)0x40000000) + 0x00012400)));
ADC_ClearITPendingBit(((ADC_TypeDef *) (((uint32_t)0x40000000) + 0x00012400)), ((uint32_t)0x00000004));
}
...
void TIM2_IRQHandler(void){
  if(TIM_GetITStatus(__ICtimerName,((uint16_t)0x0001))){
      TIM_ClearITPendingBit(__ICtimerName, ((uint16_t)0x0001));
    updateCounter++;
