...
void dac::begin(uint8_t chan){
  ...
  pinMode(((GPIO_TypeDef *) ((((uint32_t)0x40000000) + 0x08000000) + 0x00000000)),4+channel,GPIO_Mode_AN);
 
   
  RCC_APB1PeriphClockCmd(((uint32_t)0x20000000), ENABLE);
  
   
  DAC_InitStructure.DAC_Trigger = ((uint32_t)0x00000000);
  DAC_InitStructure.DAC_WaveGeneration = ((uint32_t)0x00000000);
  DAC_InitStructure.DAC_LFSRUnmask_TriangleAmplitude = ((uint32_t)0x00000000);
  DAC_InitStructure.DAC_OutputBuffer = ((uint32_t)0x00000000);

  DAC_Init(((uint32_t)0x00000010)&(chan<<1), &DAC_InitStructure);
  DAC_Cmd(((uint32_t)0x00000010)&(chan<<1), ENABLE);
  
  DAC_SoftwareTriggerCmd(((uint32_t)0x00000010)&(chan<<1),ENABLE);
}
...
void dac::out(uint16_t data){
  if(channel==0){DAC_SetChannel1Data(((uint32_t)0x00000000), data);}
  else{DAC_SetChannel2Data(((uint32_t)0x00000000), data);}
}
